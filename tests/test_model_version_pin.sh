#!/usr/bin/env bash
# test_model_version_pin.sh
# agents.local.json で opus を pin
set -euo pipefail
...
{ "model_versions": { "opus": "claude-opus-4-7" } }
EOF
...
if [ "$resolved" = "claude-opus-4-7" ]; then
  pass=$((pass+1)); echo "  PASS: opus pinned to claude-opus-4-7"
else
  fail=$((fail+1)); echo "  FAIL: got $resolved"
fi
